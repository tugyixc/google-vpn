...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.warpconfig.vfzkqm"
    minSdk = 24
    targetSdk = 35
    versionCode = 1
    versionName = "1.0"
...
afterEvaluate {
  ...
  tasks.findByName("assembleDebug")?.apply {
    doLast {
      ...
      if (apkFile.exists()) {
        val rootDest = File(rootDirFile, "Phoenix_VPN_debug.apk")
        apkFile.copyTo(rootDest, overwrite = true)

        val appDest = File(projectDirFile, "Phoenix_VPN_debug.apk")
        apkFile.copyTo(appDest, overwrite = true)
      }
    }
  }
